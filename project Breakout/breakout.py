...
special_effects = dict(
    long_paddle=(colors.ORANGE4,
                 lambda g: g.paddle.bounds.inflate_ip(c.platform_width // 2, 0),
                 lambda g: g.paddle.bounds.inflate_ip(-c.platform_width // 2, 0)),
    slow_ball=(colors.GREEN4,
               lambda g: g.change_ball_speed(-1),
               lambda g: g.change_ball_speed(1)),
    tripple_points=(colors.BLUE8,
...
class Breakout(Game):
    def __init__(self):
        adder_points = int(random.uniform(7,15))
        Game.__init__(self, 'Game', c.screen_width, c.screen_height, c.background_image, c.frame_rate)
        ...
        self.create_objects()
        self.points_per_brick = adder_points

    
    def add_life(self):
        adder_lifes = random.uniform(1,2)
        self.lives += int(adder_lifes)
    ...
    def create_paddle(self):
        paddle = Paddle((c.screen_width - c.platform_width) // 2,
                        c.screen_height - c.platform_height * 2,
                        c.platform_width,
                        c.platform_height,
                        c.platform_color,
                        c.platform_speed)
    ...
    def handle_ball_collisions(self):
        ...
        adder_speed = int(random.uniform(3,5))
        s = self.ball.speed
        ...
        if self.ball.top > c.screen_height:
            if balls_counter >1:
                ...
                self.lives -= 1
                balls_counter -= 1
            if self.lives == 0 and balls_counter == 0:
                self.game_over = True
            else:
                self.create_ball()
